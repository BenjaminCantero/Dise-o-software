...
def listar_salas(db: Session = Depends(get_db)):
    return sala_service.listar_salas(db)
...
def create_sala(sala: SalaIn, db: Session = Depends(get_db)):
    try:
        return sala_service.crear_sala(db, sala)
